...
import { ANCHOR_BASKET } from "../../../../lib/catalog.ts";
import { validateEvidenceFileDescriptor } from "../../../../lib/admin-upload.ts";
import { isAdminAuthorized } from "../../../../lib/admin-auth.ts";
import { isSupabaseConfigured } from "../../../../lib/env.ts";
import {
  saveObservation,
  getRecentStoredObservations
} from "../../../../lib/admin-observation-storage.ts";
import { getStoreById, isAllowedSourceUrl } from "../../../../lib/source-policy.ts";
import type {
  ComparabilityGrade,
  MeasurementUnit,
  PriceObservation,
...
export async function GET(request: Request) {
  return handleObservationRequest("GET", request);
}

export async function POST(request: Request) {
  return handleObservationRequest("POST", request);
}

export async function handleObservationRequest(
  method: "GET" | "POST",
  request: Request,
  deps: ObservationRouteDeps = defaultDeps
) {
  ...
  if (method === "GET") {
    const observations = await deps.getRecentStoredObservations(25);
  ...
  const savedObservation = await deps.saveObservation(observation, evidenceFile);

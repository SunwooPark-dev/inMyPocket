...
- For non-trivial work, default to harness engineering: freeze objective, scope, constraints, and definition_of_done first, then derive a task graph, work packets, merge gate, and verification target before implementation.
...
- For non-trivial work, define `spec`, `task graph`, `work packets`, `merge gate`, and `verification target` before implementation.
- Default review lane for parallel work is `validator + critic -> integrator`; do not stop at raw subagent outputs.
- Run lint, typecheck, tests, and static analysis after changes.
- Final reports must include changed files, simplifications made, and remaining risks.

## Codex App global default workflow policy
Applies as the default for future Codex App task packets in this scope. This is a strong default, not a higher-priority override over narrower repo/task instructions.

- Baseline workflow: `harness-engineering-development`
- Default execution pattern: `plan -> parallel_execute -> cross_verify -> integrate -> finalize`
- Default role map:
  - planner: `architect` + `harness-engineer`
  - executors: `executor[*]` with parallel fan-out first
  - validators: `verifier`
  - critics: `critic` and `security-reviewer` when risk warrants
  - integrator: `merge-coordinator` (leader fallback only when the coordinator surface is unavailable)
- Parallel-first rule: attempt bounded parallel decomposition before implementation-heavy work; if any span must remain sequential, state the reason and keep that span minimal.
- Cross-verification is required before final output; do not treat a raw bundle of lane outputs as complete.
- At the start of each Codex App task response, briefly confirm whether this default is active or whether a narrower scoped instruction replaced part of it.
- Auto-injection source of truth: `.codex/policies/harness_parallel_agents_core.json` and `.codex/templates/dispatch_packet.default.json`

## Global workflow default
This is the global default for future Codex App work in this workspace. It is a strong default, not an absolute override: system/developer/user instructions, deeper `AGENTS.md` files, safety constraints, and irreducible task dependencies still win.

Default orchestration pattern:
`plan -> parallel_execute -> cross_verify -> integrate -> finalize`

Default roles:
- `planner_agent` ? freeze objective/scope/constraints/definition_of_done and build the task graph
- `executor_agents` ? execute the maximum useful set of independent work packets in parallel
- `validator_agent` ? confirm output correctness against explicit gates
- `critic_agent` ? challenge risk, omissions, and contract drift
- `integrator_agent` ? merge verified outputs into one coherent final result

Harness requirements:
- Start non-trivial work with `objective -> scope -> constraints -> definition_of_done -> task graph -> work packets`.
- Fan out to the maximum useful set of relevant independent agents within concurrency, safety, and shared-file constraints.
- If work must shrink to partial sequential execution, record the dependency or conflict that forced it.
- Final outputs must be cross-verified and integrated; do not present raw parallel result bundles as the final answer.
...
Global default for non-trivial work: use a harness-first orchestration frame before implementation.
- Freeze `objective`, `scope`, `constraints`, `definition_of_done`, `task graph`, and `work packets` first.
- Preferred execution lane is `planner -> parallel executors -> validator/critic -> integrator`.
- Collapse to single-agent or mostly sequential execution only when the task is trivial or parallel decomposition is not materially useful; record why when that happens.
...
Default verification shape for non-trivial work:
- `validator` and `critic` should cross-check executor outputs in parallel when feasible.
- `integrator` should finalize the unified result only after validation findings are triaged.
- If sequential fallback was required, include the reason in the verification evidence.
...
- For non-trivial work, begin with harness decomposition: `objective -> scope -> constraints -> definition_of_done -> task graph -> work packets -> merge gate`.
...
- If RTK is configured, prefer OMX surfaces first for repository lookup, bounded shell-native inspection, and harness-driven workflows.
- Prefer `rtk ...` only for direct raw shell commands whose normal output would be verbose (`git`, `ls`, `cat`, `grep`, build/test runners). If the current Codex session rejects `rtk ...` by policy, immediately fall back to the equivalent raw read-only command instead of retry loops.
...
- Final outputs should be integrated results after validator/critic review, not raw subagent dumps.

Parallelization:
- Run independent tasks in parallel.
- Default execution pattern: `plan -> parallel_execute -> cross_verify -> integrate -> finalize`.
- Use the maximum useful set of relevant independent agents, capped by runtime limits, safety constraints, and shared-file conflict risk.
- Before fan-out, note likely shared-file conflicts; if only part of the graph can run in parallel, isolate the sequential edge and say why.
...
@C:\Users\sunwo\.codex\RTK.md

<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1776222702773-8t7u4d | 2026-04-15T03:11:47.107Z

**Active Modes:**
- skill-active: iteration 67/?
